...
for mouse in mouse_list:
    ...
    if mouse in IDs_all['list_WT']:
        ...
        for structure in data.keys():
            if iterator_WT == 0:
                grandAverage_WTs[structure] = np.mean(data[structure], axis=2)
            else:
                grandAverage_WTs[structure] = np.dstack((grandAverage_WTs[structure], np.mean(data[structure], axis=2)))
     
        iterator_WT += 1

print("Keys: {}".format(grandAverage_WTs.keys()))
print("mPFC: {}".format(grandAverage_WTs['mPFC'].shape))

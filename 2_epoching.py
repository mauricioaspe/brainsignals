...
baselines_all = pickle.load(open('DATA/ALL/npys/baselines.dict', 'rb'), encoding='latin1')

# Defining time windows as 3 seconds before plus 3 second after center entrances
fs = 30000
secs = 3
pre_window = int(fs) * secs
post_window = int(fs) * 2
...
for mouse in IDs.keys():
    ...
    for structure in structures.keys():
        ...
        data_epochs    = np.zeros((n_channels, pre_window + post_window, n_epochs))
        print("Empty data epoch shape: {}".format(data_epochs.shape))
        data_baselines = np.zeros((n_channels, pre_window, n_baselines))
        ...
        for channel in range(n_channels):
            for epoch in range(n_epochs):
                ...
                print("Pre window: {}".format(pre_window))
                print("Post window: {}".format(post_window))                
                data_epochs[channel, :, epoch] = data[channel, entrances_times[epoch] - pre_window : entrances_times[epoch] + post_window]
            
            for baseline in range(n_baselines):
                data_baselines[channel, :, baseline] = data[channel, baselines[baseline] : baselines[baseline] + pre_window]
